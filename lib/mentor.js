...
mentor.prototype.learn = function(){
	...
	var _finalstate = this.lessons.states[this.lessons.states.length-1].toString();
	...
	this.lessons.states.forEach(function(s){
		...
		Object.keys(s.obs).forEach(function(e){
			...
			_emissions[e.toString()] = s.obs[e.toString()];
		})
...
mentor.prototype.verifyLessons = function(callbackEach){
	...
	self.lessons.states.forEach(function(s){
		...
		if (sumOfTrans!=1.0 || sumOfTrans!=0.0){
			callbackEach.apply(self,['Total sum of transition prob from ['+s.state+'] not equal to 0 or 1 (got '+sumOfTrans+')']);
			result &= 0;
		}
		...
		if (sumOfObs!=1.0 || sumOfObs!=0.0){
			callbackEach.apply(this,['Total sum of the emission prob of [' +s.state +'] not equal to 0 or 1 (got +'+sumOfObs+')']);
		}
	});
...
mentor.prototype.predictLabel = function(chain){
	...
	var resultPath = this.hidden_markov.viterbiApproximation(chain);
