...
function initDatabase(dbName,collectionName) {
	...
	self.db.once('open', function() {  
	    self.lessonSchema = new self.mongoose.Schema({  
	        _id: self.mongoose.Schema.ObjectId,  
	        labels: [],
	        states: [stateSchema] 
	    });  
...
mentor.prototype.save = function(){
	// Exit with failure if there is no open connection
	if (typeof(this.db)=='undefined'){
		console.error('@Mentor is unable to save the lessons due to db connectivity.');
		return false;
	}

	// Save the lessons now
	// TAOTODO:
}
