...
mentor.prototype.learn = function(){
	...
	var states = this.lessons.states.map(function(el){return el.state});
	var finalstate = this.lessons.states[this.lessons.states.length-1];
	...
	this.lessons.states.forEach(function(s){
		...
		Object.keys(s.obs).forEach(function(e){
			...
			emissions[e.toString()] = s.obs[e.toString()];
		})


		transitions[s.state.toString()] = transObj;
		emissions[s.state.toString()] = emissObj;
	});



	var _hmm = new this.hmm(states, finalstate, labels, initialProbs, transitions, emissions);
	_hmm.print();

	// TAOTODO: Store the learned HMM
}
